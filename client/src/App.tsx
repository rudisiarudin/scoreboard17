...
export default function App() {
  ...
  // hanya operator yang push; hindari loop
  useEffect(() => {
  ...
  // adakah skor non-nol di mana pun?
  const globalHasAnyScore = Object.values(state.scores || {}).some((row) =>
    Object.values(row || {}).some((v) => (v ?? 0) > 0)
  );

  // event aktif (jika operator memilih tab event)
  const activeEvent = state.currentEventId ? state.events.find((e) => e.id === state.currentEventId) : null;

  // tampil roster jika:
  // - ada event aktif namun event itu belum ada nilai, ATAU
  // - di klasemen (tidak ada event aktif) dan belum ada nilai sama sekali
