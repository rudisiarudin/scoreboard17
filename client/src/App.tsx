...
export default function App() {
  ...
  // persist ke localStorage + siarkan versi (lintas-tab lokal)
  useEffect(() => {
    saveState(state);
    try {
  ...
  // confetti ketika pemimpin berubah
  useEffect(() => {
  ...
  /* ================= Supabase Realtime ================= */
  // Operator: push (upsert) setiap state berubah (debounce 300ms)
  useEffect(() => {
    if (audienceMode) return; // hanya operator
    const id = setTimeout(() => {
      supabase.from("board").upsert({ id: ROW_ID, state, version: state.version, updated_at: new Date().toISOString() });
    }, 300);
    return () => clearTimeout(id);
  }, [state, audienceMode]);

  // Audience: bootstrap + subscribe realtime
  useEffect(() => {
    if (!audienceMode) return;

    let alive = true;
    const apply = (next: BoardState) => alive && setState(next);
    ...
    supabase
      ...
      .eq("id", ROW_ID)
      .single()
      .then(({ data }) => { if (data?.state) apply(data.state as BoardState); });

    // subscribe perubahan postgres
    ...
    // fallback local sinkronisasi lintas-tab
    ...
    return () => {
      ...
      if (bcRef.current) bcRef.current.onmessage = null as any;
        ...
        <div className="relative mx-auto w-full max-w-[120rem] px-4 lg:px-8 py-6">
          ...
          <div className="fixed right-4 bottom-4 z-40">
            <div className="flex items-center gap-2 bg-white/90 backdrop-blur px-3 py-2 rounded-2xl border border-red-200 shadow-lg">
              <a href="#" className="px-3 py-1.5 rounded-xl border border-red-200 bg-white text-red-700 hover:bg-red-50">Keluar Mode Penonton</a>
              <button onClick={toggleFullscreen} className="px-3 py-1.5 rounded-xl border border-red-200 bg-white text-red-700 hover:bg-red-50">{fullscreen ? "Keluar Fullscreen" : "Full Screen"}</button>
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="relative mx-auto w-full max-w-[120rem] px-4 lg:px-8 py-6">
          ...
          <div className="mt-6 flex w-full justify-center">
            <div className="flex max-w-full flex-wrap items-center justify-center gap-3 overflow-x-auto no-scrollbar px-1">
              <TabButton active={activeTab === "summary"} onClick={() => setActiveTab("summary")}>Klasemen</TabButton>
              {state.events.map((e) => (
                <TabButton key={e.id} active={activeTab === e.id} onClick={() => setActiveTab(e.id)}>{e.name}</TabButton>
              ))}
            </div>
          </div>
          ...
          <div className="fixed right-4 bottom-4 z-40">
            <div className="flex items-center gap-2 bg-white/90 backdrop-blur px-3 py-2 rounded-2xl border border-red-200 shadow-lg">
              <a href="#audience" className="px-3 py-1.5 rounded-xl border border-red-200 bg-white text-red-700 hover:bg-red-50">Mode Penonton</a>
              <button onClick={toggleFullscreen} className="px-3 py-1.5 rounded-xl border border-red-200 bg-white text-red-700 hover:bg-red-50">{fullscreen ? "Keluar Fullscreen" : "Full Screen"}</button>
            </div>
          </div>
        </div>
...
function TabButton({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-2xl border whitespace-nowrap transition-transform duration-200 ${active ? "!bg-red-600 !text-white !border-red-600" : "!bg-white !text-red-700 !border-red-300 hover:!bg-red-50"} hover:scale-[1.03] active:scale-[0.98]`}>
      {children}
    </button>
  );
}
    ...
    <div className="relative w-full mt-6">
      <div className="flex items-center justify-between px-8">
        ...
        <div className="text-center">
          ...
          <h1 className="mt-3 font-extrabold text-red-700 drop-shadow-md leading-tight" style={{ fontSize: "clamp(2rem, 2.8vw, 3.2rem)" }}>BERSATU KITA GESIT</h1>
          <p className="mt-2 text-neutral-800 text-[13px] md:text-sm max-w-xl mx-auto backdrop-blur px-3 py-1.5">Mari kita bergerak, berinovasi, dan berkolaborasi untuk mencapai tujuan bersama.</p>
        </div>
