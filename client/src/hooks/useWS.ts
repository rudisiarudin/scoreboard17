...
export function useWS(onRemote: (s: BoardState)=>void, current: BoardState) {
  ...
  useEffect(()=>{ let retry=0, hb:any=null;
    function connect() {
      ...
      ws.onmessage=(ev)=>{ try{ const d=JSON.parse(ev.data);
        if (d?.type==="state") { if (d.source===clientId.current) return; suppress.current=true; onRemote(d.payload as BoardState); }
      }catch{} };
      const down=()=>{ clearInterval(hb); setStatus("offline"); const delay=Math.min(2000*2**retry,15000)+Math.random()*500; retry++; setTimeout(connect, delay); };
  ...
  function broadcast(state: BoardState) {
    ...
    deb.current=setTimeout(()=>{ wsRef.current?.readyState===1 && wsRef.current.send(JSON.stringify({type:"state", payload: state, source: clientId.current})); }, 80);
  }
